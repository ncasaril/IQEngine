...
class usb_receiver(Plugin):
    """Upper-sideband SSB demodulator. Tune target_freq so the suppressed
    carrier sits at the lower edge of the audio band (just below the signal)."""

    sample_rate: int = 0
    center_freq: int = 0

    # custom params
    target_freq: float = 0.0  # Hz offset; place suppressed carrier at 0 Hz
    audio_bandwidth: float = 3_000.0  # typical voice SSB passband
    gain: float = 1.0
    ...
    def rf_function(self, samples, job_context=None):
        ...
        # LPF the complex baseband to the audio bandwidth (removes out-of-band noise)
        ...
        # SSB demod: real part of the analytic complex baseband is the audio
